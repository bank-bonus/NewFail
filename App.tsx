...
import { Play, Home, RefreshCw, ShoppingCart, Heart, Star, Settings, Pause, X, RotateCcw, Clapperboard, Award, Shield, Zap, Tv, Film, Trophy, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function App() {
    ...
    const [activeModal, setActiveModal] = useState<{type: 'confirm' | 'success', item: any} | null>(null);
    ...
    const tryBuyItem = (item: any) => {
        if (stats.totalStars < item.price) return;
        setActiveModal({ type: 'confirm', item });
    };
    ...
    const confirmPurchase = () => {
        ...
        // Item effects logic
        if (item.id === 'shield') {
            // Placeholder: shield could be consumed in game start logic
        }

        setActiveModal({ type: 'success', item });
    };
    ...
    const Modal = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[300] flex items-center justify-center p-6">
                <div className="max-w-xs w-full bg-soviet-cream border-4 border-black p-6 rounded-[32px] shadow-hard-lg text-center animate-slide-up relative">
                    <div className="mb-4 flex justify-center">
                        {type === 'confirm' ? <AlertTriangle size={64} className="text-soviet-gold" /> : <CheckCircle size={64} className="text-soviet-green" />}
                    </div>
                    <h2 className="font-ruslan text-2xl mb-2 text-soviet-dark uppercase">
                        {type === 'confirm' ? T.purchase_confirm : T.bought_success}
                    </h2>
                    <div className="bg-white/50 border-2 border-black/10 rounded-2xl p-4 mb-6">
                         <div className="flex items-center justify-center gap-3 mb-1">
                            <item.icon size={24} className="text-soviet-red" />
                            <span className="font-bold text-lg">{item.name[lang]}</span>
                         </div>
                         <p className="text-xs text-gray-500 italic">{item.desc[lang]}</p>
                    </div>
                    
                    <div className="flex flex-col gap-3">
                        {type === 'confirm' ? (
                            <>
                                <Button fullWidth rounded onClick={confirmPurchase} variant="primary">
                                    {T.earn} ({item.price} ⭐)
                                </Button>
                                <Button fullWidth rounded onClick={() => setActiveModal(null)} variant="secondary">
                                    {T.close}
                                </Button>
                            </>
                        ) : (
                            <Button fullWidth rounded onClick={() => setActiveModal(null)} variant="accent">
                                {T.close}
                            </Button>
                        )}
                    </div>
